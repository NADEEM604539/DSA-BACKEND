...
def member_search(q: str, db: Session = Depends(get_db)):
    """
    Generic search by title or author using DSA structures.
    Combines Trie prefix searches for title and author.
    """
    # Search by title using Trie (O(m) where m = query length)
    title_results = search_by_title(db, q)
    
    # Search by author using Author Trie (O(m) where m = query length)
    author_results = search_by_author(db, q)
    
    # Combine results and remove duplicates based on book_id
    combined = {}
    for book in title_results:
        combined[book.get("book_id", id(book))] = book
    ...
    return {
        "books": results,
        "search_type": "Combined Trie Search (Title + Author)",
        "results_count": len(results)
    }
