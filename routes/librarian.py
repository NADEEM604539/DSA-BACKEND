...
from core.synchronizer import add_book, delete_book, update_book
...
def librarian_add_book(
    ...
    Updates both database AND DSA structures (Trie, AVL, Hash Table).
    """
    try:
        from models import Author, Category
        from services.library import add_new_book
        ...
        # Add to both DB and DSA structures (Trie, AVL, Hash Table)
        add_book(db, book)
        
        return {
            "message": "Book added successfully to database and DSA structures",
...
def librarian_update_book(
    ...
    try:
        ...
        # Prepare update data and use synchronizer
        update_data = {}
        if book_data.title:
        ...
        if book_data.year:
            update_data['publication_year'] = book_data.year
        if book_data.quantity is not None:
            update_data['total_copies'] = book_data.quantity
        if book_data.available is not None:
            update_data['available_copies'] = book_data.available
        if 'author_id' in vars(book):
            update_data['author_id'] = book.author_id
        if 'category_id' in vars(book):
            update_data['category_id'] = book.category_id
        
        # Use synchronizer to update with DSA structures
        if update_data:
            update_book(db, book, update_data)
        else:
            db.commit()
...
def librarian_delete_book(book_id: int, db: Session = Depends(get_db)):
    """
    Delete a book from the system using DSA structures.
    """
    book = db.query(Book).filter(Book.book_id == book_id).first()
    if not book:
        raise HTTPException(status_code=404, detail="Book not found")
